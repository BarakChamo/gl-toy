...
class ShaderFrame {
	constructor({ tex, background = transparentPixel, fx, canvas, params = {}, updateHook = noOp, debug = false,  clear = false, onClear = noOp }) {
		this.position = 0.0
		this.target = 0.0
		this.duration = 1.0
		...
		const containsCanvas = true //document.contains(canvas)
		const containsTex = true //document.contains(tex)
		...
		this.loadedPromise = new Promise(resolve => {
			this._resolve = () => {
				if(debug) console.log('Shader Frame: resolving load promise.')
				this._loaded = true
				resolve(true)
			}
		})
	...
	**/ 
	...
	fit = () => {
		...
		const imageInDocument = true //document.contains(this.texElm)
